...
def plot_field_1d(knots, degree, u, nx, color='b'):
    ...
    #plt.plot(xs, Q[:,0],label='p=%1d ' %degree)
    plt.plot(xs, Q[:,0],label='U approached ')    
    plt.legend()
    plt.grid()
    
 #==============================================================================
def plot_field_2d(Exact_Solution, knots1,knots2, degree1, degree2, u, nx,ny, color='b'):
    n1 = len(knots1) - degree1 - 1
    n2 = len(knots2) - degree2 - 1
    xmin1 = knots1[degree1]
    xmax1 = knots1[-degree1-1]
    
    xmin2 = knots2[degree2]
    xmax2 = knots2[-degree2-1]

    xs1 = np.linspace(xmin1, xmax1, nx)
    xs2 = np.linspace(xmin2, xmax2, ny)

    P = np.zeros((n1, n2,1))
    
    P[:,:,0] = u[:,:]
    Q = np.zeros((nx,ny, 1))
    Exact_Sol = np.zeros((nx,ny))
    for i,x in enumerate(xs1):
        for j,y in enumerate(xs2): 
            Q[i,j,:] = point_on_bspline_surface(knots1, knots2, P, x, y)
            Exact_Sol[i,j] = Exact_Solution(x,y)
    fig=plt.figure()
    ax=fig.gca(projection='3d')
    X,Y=np.meshgrid(xs1,xs2)
    Z=Q[:,:,0]
    Z2=Exact_Sol[:,:]
    #print('Error L-infty=',np.max(np.absolute(Z-Exact_Sol)))
    # Plot the surface.
...
def separation(V):
    ...
    for i in range(n):
        A[i]=V[i].real
        B[i]=V[i].imag
        C[i]=np.sqrt(A[i]*A[i]+B[i]*B[i])
    return A,B,np.squeeze(C)
#==============================================================================
def tri(V):
    n=len(V)
